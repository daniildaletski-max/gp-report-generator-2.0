...
export async function getAuditLogStats() {
  ...
  const [totalLogs, todayLogs, weekLogs, failedLogs] = await Promise.all([
    db.select({ count: sql<number>`COUNT(*)` }).from(auditLogs),
    db.select({ count: sql<number>`COUNT(*)` }).from(auditLogs).where(gte(auditLogs.createdAt, dayAgo)),
    db.select({ count: sql<number>`COUNT(*)` }).from(auditLogs).where(gte(auditLogs.createdAt, weekAgo)),
    db.select({ count: sql<number>`COUNT(*)` }).from(auditLogs).where(eq(auditLogs.status, 'failure')),
  ]);
...
const RATE_LIMIT_WINDOWS = {
  ...
  llm: { maxRequests: 30, windowMs: 60 * 60 * 1000 }, // 30 LLM calls per hour
  bulk: { maxRequests: 10, windowMs: 60 * 60 * 1000 }, // 10 bulk operations per hour
  delete: { maxRequests: 50, windowMs: 60 * 60 * 1000 }, // 50 deletes per hour
};

export async function checkRateLimit(
  userId: number, 
  endpoint: keyof typeof RATE_LIMIT_WINDOWS
): Promise<{ allowed: boolean; remaining: number; resetAt: Date }> {
  const db = await getDb();
  if (!db) {
    // If database is unavailable, allow the request but log warning
    console.warn("[RateLimit] Database not available, allowing request");
    return { allowed: true, remaining: 999, resetAt: new Date() };
  }

  const config = RATE_LIMIT_WINDOWS[endpoint];
  ...
  const remaining = config.maxRequests - record.requestCount - 1;
...
export async function resetRateLimit(userId: number, endpoint: string): Promise<void> {
  const db = await getDb();
  if (!db) return;
