...
async function generateExcelAndEmail(ctx: { user: { id: number; role: string; email?: string | null; name?: string | null } }, reportId: number) {
  console.log(`\n\n========== [exportToExcel] START ==========`);
  console.log(`[exportToExcel] Called with input.reportId=${reportId}`);
  ...
  console.log(`[exportToExcel] reportId=${reportId}, teamId=${report.teamId}, year=${report.reportYear}, month=${report.reportMonth}`);
  ...
  ["N36", "Q36", "S36", "U36", "W36", "Y36"].forEach(cell => {
    mainSheet.getCell(cell).border = {
      top: { style: "thin" }, left: { style: "thin" },
      bottom: { style: "thin" }, right: { style: "thin" }
    };
  });

  // Column N borders for the entire attendance table
  ...
  if (ctx.user.email && shouldSendEmail) {
    ...
    console.log(`[exportToExcel] Existing Excel detected; skipping duplicate email.`);
  }
